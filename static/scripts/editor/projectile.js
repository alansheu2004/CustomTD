//Tower Table
var projectileTable = document.getElementById("projectileTable");
var projectileTableTemplate = document.getElementById("projectileTableTemplate");
//var backtoTowerScreen = document.getElementById("backToTowerScreen");
function setUpProjectileTable() {
    fillProjectileRows();
    //backtoProjectileScreen.addEventListener("click", exitProjectile);
}

function fillProjectileRows() {
    projectileTable.textContent = "";
    for(let projectile of currentState.game.projectileTypes.filter(function(projectile) {return projectile.base==null})) {
        let row = projectileTableTemplate.cloneNode(true);
        row.id = "";
        row.style.display = "table-row";
        row.children[0].children[0].src = projectile.image.src;
        row.children[1].textContent = projectile.name;
        ...
        projectileTable.appendChild(row);
    }
}
